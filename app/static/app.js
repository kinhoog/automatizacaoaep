(() => {
  ...
  function renderValidation(snapshot) {
    ...
    if (reportUrl) {
      earlyReportLink.href = reportUrl;
  ...
  function showCompleted(snapshot) {
    ...
    documentLink.href = downloads.document || "#";
    reportLink.href = downloads.validation_report || "#";
    documentLink.toggleAttribute("aria-disabled", !downloads.document);
    reportLink.toggleAttribute("aria-disabled", !downloads.validation_report);
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (error instanceof ApiError) {
        ...
        showError(
          "Não foi possível conectar ao serviço local. Confirme se o servidor está em execução.",
        );
      }
    } finally {
  ...
  earlyReportLink.addEventListener("click", (event) => {
    if (earlyReportLink.hidden || earlyReportLink.href.endsWith("#")) {
      event.preventDefault();
    }
  });
